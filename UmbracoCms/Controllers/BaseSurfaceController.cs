...
namespace UmbracoCms.Controllers
{
    public class BaseSurfaceController : SurfaceController
    {
        ...
        {
            var enableDebug = System.Configuration.ConfigurationManager.AppSettings["Umbraco.Web.Mvc.Surface.DebugEnable"].ToString().ToLower() == "true";
            if (enableDebug)
            {
                ...
                LogHelper.Info<BaseSurfaceController>(string.Format("From the controller: '{0}', action: '{1}', class: '{2}'.", controllerName, actionName, className));                
